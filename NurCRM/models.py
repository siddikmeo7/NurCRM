...
class ShopOrder(models.Model):
    ...
    def __str__(self):
        return f"Shop Order #{self.id} - {self.status}"
    

class TransactionOrder(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name="transaction_orders")
    transaction_type = models.CharField(max_length=10, choices=[('loan', 'Loan'), ('repay', 'Repay')])
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    comments = models.TextField(blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Transaction Order #{self.id} - {self.transaction_type} - {self.amount}"